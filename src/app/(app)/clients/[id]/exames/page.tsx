...
const FLAG_BADGE: Record<string, string> = {
  ...
  ATENCAO: "badge-danger",
  INDETERMINADO: "badge-info",
};

export default async function ExamResultsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const data = await getExamResultsGrouped(id);
  if (!data) notFound();

  const { client, parameters } = data;
  const attention = parameters.filter((p) => p.flag === "ATENCAO");

  return (
    <div className="animate-in">
      <Link href={`/clients/${client.id}`} className="btn btn-ghost btn-sm" style={{ marginBottom: 16 }}>
        ← Voltar para o paciente
      </Link>

      <div className="page-header">
        <div>
          <h1>Histórico de exames — {client.name}</h1>
      ...
      {parameters.length === 0 ? (
        <div className="card">
          <div className="empty-state">
            ...
            <p>Nenhum resultado de exame importado ainda.</p>
          </div>
        </div>
      ) : (
